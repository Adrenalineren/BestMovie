...
const getScreeningManagement = async (req, res) => {
  ...
  res.render('screening/screening-management', { user: req.session.user, screenings: populatedScreenings, error: undefined, success: successMessage });
};
...
const getScreeningCreate = async (req, res) => {
  ...
  res.render('screening/screening-create', { user: req.session.user, screening: null, movies: allMovies, halls: activeHalls, error: undefined });
};
...
const postScreeningCreate = async (req, res) => {
  ...
  try {
    ...
    if (minutesToTime(endMinutes) > '23:59') {
      return res.status(400).type('text/plain').send('Screening time exceeds the maximum allowed time.');
    } 
...
const getScreeningEdit = async (req, res) => {
  ...
  res.render('screening/screening-create', {
...
const postScreeningEdit = async (req, res) => {
  ...
  try {
    ...
    req.session.successMessage = 'Screening updated successfully!';
    res.redirect('/admin/screening/screening-management');
  } catch (err) {
